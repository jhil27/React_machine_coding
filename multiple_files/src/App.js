...
import './App.css';
import { stockMetrics } from './data';
import { useState } from 'react'
import FoldersNFiles from './foldersNfiles';
function App() {
  const metrics = new stockMetrics();
  const data = metrics.fileStructures;
  const mapper = () => {
    const mainMap = new Map();
    for (let i = 0; i < data.length; i++) {
      if (mainMap.has(data[i].cat)) {
        let arr = mainMap.get(data[i].cat);
        arr.push({ ...data[i] })
        mainMap.set(data[i].cat, [...arr]);
      }
      else mainMap.set(data[i].cat, [{ ...data[i] }])
    }
    mainMap.forEach((value, key) => {
      let childMap = new Map();
      for (let i = 0; i < value.length; i++) {
        if (childMap.has(value[i].subCat)) {
          let arr = childMap.get(value[i].subCat);
          arr.push({ ...value[i] })
          childMap.set(value[i].subCat, [...arr]);
        }
        else childMap.set(value[i].subCat, [{ ...value[i] }])
      }
      mainMap.set(key, childMap)
    })
    let folders = [...mainMap];
    folders.forEach((folder) => {
      folder.push({ isOpen: false })
    });
    return folders;
  }
  const [folderStructure, setfolderStructure] = useState(mapper());
  const handleFolderClick = (index) => {
    console.log(folderStructure[index], folderStructure[index][2].isOpen)
    folderStructure[index][2].isOpen = !folderStructure[index][2].isOpen;
      ...
      <div className='mainfolder'>
        {folderStructure.map((folder, index) => {
          return <div>
            ...
            <span onClick={() => handleFolderClick(index)}>{folder[0]}</span>
            {folder[2].isOpen
              ? <FoldersNFiles files={folder[1]} />
              : <></>
            }
          </div>
        })}
